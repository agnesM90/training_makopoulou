...
interior_f <- c(187, 193, 392)
interior_c <- (interior_f - 32) * 5/9

exterior_f <- c(74, 56, 56)
#convert to celsius
exterior_c <- (exterior_f -32) * 5/9

surface_f <- c(109, 406, 34)
sueface <- (surface_f -32) * 5/9
...
convert_temps(surface_f, units = "F") #always use the units if you don't use the default unit line:34
...
add_hot_or_cold <- function(df, thresh= 70) {#df is data frame
  if (!"fahr" %in% names(df)){
    stop('Data frame must have fahr column called `fahr`!')
    }
  out_df <- df%>%
